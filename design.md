...
- ホバー: 枠線の色（`#c4d2f6`）が変化し、サムネイル画像がわずかに拡大（`@media (hover: hover)` で制御。カード自体の浮き上がりはなし）
...
- **アナリティクス・計測仕様 (`analytics-events.js`)**:
  - **到達（インプレッション）検知 (`affiliate_view`)**: `IntersectionObserver` によりカードの50%以上が画面内に入った瞬間に1回発火。
  - **クリック検知 (`affiliate_click`)**: カード内リンクのクリック時に発火。
  - **送信パラメータ**: `affiliate_title`（商品名）、`affiliate_url`（URL）、`article_title`（記事名）、`position`（`in_content` / `bottom`）、`page_path`（URLパス）。
  - **CTR算出**: `affiliate_click ÷ affiliate_view` で到達者クリック率（真のCTR）を可視化可能。
